...
func _ready():
	...
	game_timer.set_wait_time(60)  # Set the game duration to 2 minutes (120 seconds)
...
func proses_pesanan(barang_diberikan):
	...
	if input_player == kode_pesanan_saat_ini["kode"] and current_money == kembalian:
		score += 100
		GlobalScore.add_to_score(100)
	else:
		print("Pesanan salah! Kode input: ", barang_diberikan)
		score -= 50
		GlobalScore.add_to_score(-50)  # Deduct from global score
